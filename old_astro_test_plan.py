...
from agents import Equipment, infer_equipment_specs, EquipmentQuery, \
    infer_observer_context, ObserverContext, AstroDependencies, \
    db_path 
...
class Plan(BaseModel):
    ...
    # observation_date: datetime = Field(..., description="The date of the observation session")
    # location: str = Field(..., description="The location of the observation session")
...
astro_agent = Agent(
    ...
    When the user asks about planning a session, you should:
     - product a Plan object containing a list of DeepSpaceObject models to observe.
     - Use the tools to fetch data.
     - Use only the deep space object tool for deep sky object information. Do not include other sources of deep sky objects.
     - If the user specifies a telescope and or camera, use the infer_equipment_specs tool to get detailed specifications about the gear.
     - If the user provides location and date/time information, use the infer_observer_context tool to get detailed observer context.

     - Deep Space Object Table schema:
        CREATE TABLE dso (
     ...
     - If the user's plan includes telescope and or camera gear, use the tool infer_equipment_specs to get detailed specifications about the gear.

    """,
    output_type=Plan,
    tools=[infer_equipment_specs, infer_observer_context],
    retries=2,
...
async def main() -> None:
    ...
    while True:
        ...
        try:
            # get today's date with timezone America/Chicago
            deps = AstroDependencies(todays_date=datetime.datetime.now(
                datetime.timezone(datetime.timedelta(hours=-6))
            ).date()
            )
            result = await astro_agent.run(user_msg,deps=deps)
            print("Bot Output --->:")
